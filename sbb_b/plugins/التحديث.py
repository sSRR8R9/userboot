...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    if heroku_app is None:
        ...
        return repo.__del__()
    razan = await event.edit(
        "**الان يتم تنصيب تحديث ماتركس ، يرجى الانتظار حتى تنتهي العملية ، وعادة ما يستغرق التحديث من 4 إلى 5 دقائق.**"
    )
    try:
...
async def upstream(event):
    ...
    if ENV and (HEROKU_API_KEY is None or HEROKU_APP_NAME is None):
        return await edit_or_reply(
            event, "⌔∮ يجب عليك وضع الفارات المطلوبة لتحديث ماتركس"
        )
    try:
    ...
    if changelog == "" and not force_update:
        await event.edit("\n⌔∮ عزيز المستخدم انت تستخدم اخر اصدار من ماتركس - @VV744")
        return repo.__del__()
    if conf == "" and not force_update:
        await print_changelogs(event, ac_br, changelog)
        await event.delete()
        return await event.respond(f"ارسل `{cmdhd}تحديث الان` لتحديث سورس ماتركس")

    if force_update:
        await event.edit("- يتم التحديث الاجباري لأخر اصدار من السورس انتظر قليلا")
    if conf == "الان":
        await event.edit("⌔∮ جارِ تحديث ماتركس يرجى الأنتظار قليلا")
...
async def upstream(event):
    ...
    off_repo = "https://github.com/bbhh1/matrix"
    os.chdir("/app")
    try:
        txt = "**⌔∮ عذرا لم يتم اكمال التحديث بسبب بعض الاخطاء " + "**اللوگ:**\n"
